...
print("image_shape: ",image_color.shape)

image_color = image_color.transpose(2, 0, 1)
print("image_transpose(2, 0, 1)_shape: ",image_color.shape)

batch_image_color = np.repeat(image_color[np.newaxis, :, :, :], 15, axis=0)
print("batch_image_color_shape: ", batch_image_color.shape)

W, b = init_weigit(num_filters=10, data_dim=3, kernel_size=3)
conv = Conv2D(W, b)
pool = Polling2D()

output = conv.forward(batch_image_color)
print("Conv_forward_1_shape: ", output.shape)

W2, b2 = init_weigit(num_filters=30, data_dim=10, kernel_size=3)
conv2 = Conv2D(W2, b2)
pool = Polling2D(stride=2, kernel_size=2)

output2 = conv2.forward(output)
print("Conv_forward_2_shape: ", output2.shape)

output3 = pool.forward(output2)
print("Poolling_forward_shape: ", output3.shape)
...
image_gary = url_to_image(img_url, gray=True)
image_color = url_to_image(img_url, gray=False)
image_gary = image_gary.reshape(image_gary.shape[0], -1, 1)

print("image_gray_shape: ", image_gary.shape)  # (440, 440, 1)
print("image_color_shape: ", image_color.shape)
...
image_gary = np.expand_dims(image_gary.transpose(2, 0, 1), axis=0)
image_color = np.expand_dims(image_color.transpose(2, 0, 1), axis=0)

plt.subplot(2, 3, 1)
plt.title("GrayImage")
plt.imshow(image_gary[0, 0, :, :], cmap='gray')


W, b = init_weigit(num_filters=1, data_dim=1, kernel_size=3)
conv = Conv2D(W, b)
output = conv.forward(image_gary)

print("num_filters=1, data_dim=1, kernel_size=3: ", output.shape)

plt.subplot(2, 3, 2)
plt.title("output")
plt.imshow(output[0, 0, :, :], cmap='gray')

W2, b2 = init_weigit(num_filters=1, data_dim=1, kernel_size=3, stride=2)
conv2 = Conv2D(W2, b2, stride=2)
output2 = conv2.forward(image_gary)
print("num_filters=1, data_dim=1, kernel_size=3, stride=2: ", output2.shape)

plt.subplot(2, 3, 3)
plt.title("output2")
plt.imshow(output2[0, 0, :, :], cmap='gray')

W3, b3 = init_weigit(num_filters=10, data_dim=3, kernel_size=3)
conv3 = Conv2D(W3, b3)
output3 = conv3.forward(image_color)
print("num_filters=1, data_dim=1, kernel_size=3, stride=2: ", output3.shape)
